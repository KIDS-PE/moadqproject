...
n_iter<-nrow(accuracy_rule)
pb <- tkProgressBar(title = "Checking accuracy", label = "Percentage completed",
                    min = 0, max = n_iter,initial = 0, width = 500)


for(i in accuracy_rule$rule_id){
  ...
  if(tmp2$rule=='date accuracy'){
    ...
    if(length(error_message)>0){accuracy_result$result[tmp1]<-paste(error_message, collapse = ';')} else{
      ...
      if(is.na(tmp7)==TRUE){accuracy_result$result[tmp1]<-NA} else{accuracy_result$result[tmp1]<-round((tmp7/nrow(tmp6))*100, 0)}
    }
  }
  ...
  if(tmp2$rule=='age accuracy'){
    ...
    if(length(error_message)>0){accuracy_result$result[tmp1]<-paste(error_message, collapse = ';')} else{
      ...
      if(is.na(tmp5)==TRUE){accuracy_result$result[tmp1]<-NA} else{accuracy_result$result[tmp1]<-round((tmp5/count_rows)*100, 4)}
    }
  }
  ...
  if(tmp2$rule=='vital value range accuracy'){
    ...
    if(tmp3!=TRUE){accuracy_result$result[tmp1]<-paste(error_message, collapse = ';')} else{
